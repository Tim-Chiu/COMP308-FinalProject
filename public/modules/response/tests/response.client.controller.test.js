...
(function() {
	...
	describe('Responses Controller Tests', function() {
		...
		// the response exactly. To solve the problem, we define a new toEqualData Jasmine matcher.
		...
		it('$scope.find() should create an array with at least one Response object fetched from XHR', inject(function(Responses) {
			...
			// Set GET response
			$httpBackend.expectGET('response').respond(sampleResponses);
			...
			// Test scope value
			expect(scope.response).toEqualData(sampleResponses);
		}));
		...
		it('$scope.findOne() should create an array with one Response object fetched from XHR using a responseId URL parameter', inject(function(Responses) {
			...
			// Set GET response
			$httpBackend.expectGET(/response\/([0-9a-fA-F]{24})$/).respond(sampleResponse);
		...
		it('$scope.create() with valid form data should send a POST request with the form input values and then locate to new object URL', inject(function(Responses) {
			...
			// Set POST response
			$httpBackend.expectPOST('response', sampleResponsePostData).respond(sampleResponseResponse);
			...
			expect($location.path()).toBe('/response/' + sampleResponseResponse._id);
		}));
		...
		it('$scope.update() should update a valid Response', inject(function(Responses) {
			...
			// Set PUT response
			$httpBackend.expectPUT(/response\/([0-9a-fA-F]{24})$/).respond();
			...
			expect($location.path()).toBe('/response/' + sampleResponsePutData._id);
		}));
		...
		it('$scope.remove() should send a DELETE request with a valid responseId and remove the Response from the scope', inject(function(Responses) {
			...
			scope.response = [sampleResponse];

			// Set expected DELETE response
			$httpBackend.expectDELETE(/response\/([0-9a-fA-F]{24})$/).respond(204);
